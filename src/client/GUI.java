...
public class GUI extends Application {
    ...
    private void initiateSetup() {
        ...
        me = new Player(inputPlayerName, startPosition[0], startPosition[1], "up", "no_color");
    }
    ...
    private GridPane gridPaneSetup() {
        ...
        fireWall_right = new Image(getClass().getResourceAsStream("Image/fireWallRight.png"), size, size, false, false);
        fireWall_left = new Image(getClass().getResourceAsStream("Image/fireWallLeft.png"), size, size, false, false);
        fireWall_up = new Image(getClass().getResourceAsStream("Image/fireWallUp.png"), size, size, false, false);
        fireWall_down = new Image(getClass().getResourceAsStream("Image/fireWallDown.png"), size, size, false, false);
    ...
    private void readFromServer() {
        while (true) {
            try {
                ...
                String[] messageFormat = messageFromServer.trim().split("\\s+");
    ...
    // Game Mechanics
    private void settingUpNewPlayer(String name, int xPosition, int yPosition, String direction, String color) {
        ...
        fields[xPosition][yPosition].setGraphic(new ImageView(getHeroImage(direction, color)));
    }

    private Image getHeroImage(String direction, String color) {
        String filename = switch (direction) {
            case "right" -> color + "HeroRight.png";
            case "left" -> color + "HeroLeft.png";
            case "up" -> color + "HeroUp.png";
            case "down" -> color + "HeroDown.png";
            default -> color + "HeroUp.png";
        };
        return new Image(getClass().getResourceAsStream("Image/" + filename), size, size, false, false);
    }
    ...
    public void playerMoved(String playerName, int delta_x, int delta_y, String direction) {
        ...
        int oldX = playerToMoved.getXpos();
        int oldY = playerToMoved.getYpos();
        ...
        if (board[newY].charAt(newX) == 'w') {
            ...
            Player playerAtNewPosition = getPlayerAt(newX, newY);
            if (playerAtNewPosition != null) {
                ...
                playerToMoved.setXpos(newX);
                playerToMoved.setYpos(newY);
                playerToMoved.setDirection(direction);

                updateField(oldX, oldY);
    ...
    private void updateField(int x, int y) {
        Player playerOnField = getPlayerAt(x, y);
    ...
    private void updateScoreBoard() {
        ...
        for (Player player : players) {
            ...
            if (!player.getColor().equals("white")) playerLabel.setTextFill(Color.web(player.getColor()));
            else playerLabel.setTextFill(Color.web("black"));
